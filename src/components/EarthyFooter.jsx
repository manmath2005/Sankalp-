...
export const EarthyFooter = ({ onNavigate, onOpenDonate }) => {
  ...
  return (
    <footer className="bg-slate-950 text-slate-300 mt-20 pt-16 pb-12 border-t border-slate-800/90 relative text-left">
      <div className="horizon-gradient-line absolute top-0 left-0 right-0 h-1"></div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        
        {/* Top Newsletter & Mission Strip */}
        <div className="horizon-glass-panel p-8 rounded-3xl border border-slate-800/90 bg-slate-900/90 shadow-2xl relative overflow-hidden backdrop-blur-2xl flex flex-col lg:flex-row items-center justify-between gap-6">
          ...
          <form onSubmit={handleSubscribe} className="w-full lg:w-auto flex-1 max-w-md">
            {!isSubscribed ? (
              <div className="space-y-1.5">
                <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-slate-700/80 focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-500/20 transition-all">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 via-emerald-500 to-sky-500 hover:from-amber-400 hover:to-sky-400 text-slate-950 text-xs font-black shadow-sm transition-all press-effect shrink-0 flex items-center gap-1"
                  >
                    Subscribe
                    <Send className="w-3.5 h-3.5" />
                  </button>
                </div>
                {error && <p className="text-[11px] text-amber-400 pl-2 font-medium">{error}</p>}
              </div>
            ) : (
              <div className="flex items-center gap-2 p-2.5 rounded-2xl bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 text-xs font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-black text-lg text-white">SANKALP FOUNDATION</span>
            </div>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2 text-xs text-slate-400">
              <li>
                <button onClick={() => onNavigate('events')} className="hover:text-amber-400 transition-colors flex items-center gap-1.5">
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2 text-xs text-slate-400">
              <li>
                <button onClick={() => onNavigate('volunteer-login')} className="hover:text-amber-400 transition-colors">
                  👤 Volunteer Login &amp; Sign Up
                </button>
              </li>
              <li>
                <button onClick={() => onNavigate('company-login')} className="hover:text-amber-400 transition-colors">
                  🏢 Institutional / Company Portal
                </button>
              </li>
              <li>
                <button onClick={() => onNavigate('ngo-login')} className="hover:text-amber-400 transition-colors">
                  🛡️ NGO Partner Portal
                </button>
              </li>
              <li>
                <button onClick={() => onNavigate('volunteer-hub')} className="hover:text-amber-400 transition-colors">
                  📜 Certificate Verification Studio
                </button>
              </li>
              <li>
                <button onClick={onOpenDonate} className="hover:text-amber-300 transition-colors text-amber-400 font-bold">
        ...
        <div className="pt-8 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 text-slate-400 font-medium">
              <span>Restoring Communities</span>
              <Heart className="w-3 h-3 text-amber-400 fill-amber-400 ml-0.5" />
            </div>
          </div>
        </div>
