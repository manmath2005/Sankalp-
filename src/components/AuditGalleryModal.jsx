...
export const AuditGalleryModal = ({ event, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-xl animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl horizon-glass-panel rounded-3xl border border-slate-800/90 bg-slate-900/95 shadow-2xl overflow-hidden max-h-[92vh] flex flex-col text-left">
        <div className="horizon-gradient-line absolute top-0 left-0 right-0 h-1 z-20"></div>
        
        {/* Top Header */}
        <div className="p-5 sm:p-6 bg-slate-950/95 text-white relative border-b border-slate-800 flex items-start justify-between gap-4">
          <div className="space-y-1 pr-8">
            ...
            <p className="text-xs text-slate-400">
              Conducted by: <strong className="text-amber-400">{event.ngoName}</strong> • In Partnership with <strong className="text-sky-400">{event.partnerName}</strong>
            </p>
          </div>

          <button
            onClick={onClose}
            className="p-2 rounded-full bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white transition-colors shrink-0 border border-slate-700"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-6 pt-3 pb-2 bg-slate-950 border-b border-slate-800 flex items-center gap-2 overflow-x-auto">
          <button
            onClick={() => setActiveTab('gallery')}
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 whitespace-nowrap ${
              activeTab === 'gallery'
                ? 'bg-gradient-to-r from-amber-500 to-emerald-500 text-slate-950 shadow-xs'
        ...
        {/* Body Content */}
        <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-slate-900/90 text-slate-300">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {galleryImages.map((imgUrl, idx) => (
                  <div 
                    ...
                    className="group relative h-48 rounded-2xl overflow-hidden cursor-pointer border border-slate-800 bg-slate-950 shadow-xs hover:shadow-md transition-all"
                  >
                    <img 
                      src={imgUrl} 
                    ...
                    <span className="absolute top-2.5 left-2.5 px-2 py-0.5 rounded-md bg-slate-900/90 backdrop-blur-md text-[10px] font-mono text-slate-200 border border-slate-700">
                      Photo #{idx + 1}
                    </span>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {socialReactions.map((post) => (
                  <div 
                    key={post.id} 
                    className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3 hover:border-amber-500/50 transition-colors"
                  >
                    {/* Post Author Info */}
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full bg-amber-950/60 text-amber-300 text-[10px] font-bold border border-amber-800">
                        Verified Mention
                      </span>
                    </div>
                    ...
                    {/* Engagement Counts */}
                    <div className="flex items-center gap-4 pt-2 border-t border-slate-800 text-[11px] text-slate-400">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        <strong className="text-white">{post.shares}</strong> Reposts
                      </span>
                      <span className="flex items-center gap-1">
                        <MessageCircle className="w-3.5 h-3.5 text-amber-400" />
                        <strong className="text-white">{post.comments}</strong> Comments
                      </span>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                  ...
                  <span className="text-sm font-black text-white">{event.attendees}+ Beneficiaries</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                  <span className="text-[10px] font-bold text-slate-400 uppercase block">Volunteers</span>
                  <span className="text-sm font-black text-amber-400">{event.volunteersEngaged} Active Leads</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                  <span className="text-[10px] font-bold text-slate-400 uppercase block">Certificates</span>
                  <span className="text-sm font-black text-emerald-400">{event.certificatesIssued} Issued</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                  <span className="text-[10px] font-bold text-slate-400 uppercase block">Venue</span>
                  <span className="text-xs font-black text-white truncate block">{event.venue}</span>
                </div>
              </div>
        ...
        <div className="p-4 bg-slate-950 border-t border-slate-800 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="px-6 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs shadow-xs press-effect border border-slate-700"
          >
            Close Audit
          </button>
        </div>
